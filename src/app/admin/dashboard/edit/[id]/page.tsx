...
export default function EditProject({ params }: { params: any }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!title || !description || !slug) {
      setError("Tittel, beskrivelse og slug må fylles ut");
      return;
    }
    ...
    try {
      ...
      if (!res.ok) throw new Error("Kunne ikke oppdatere prosjekt");
      router.push("/admin/dashboard");
    } catch (err) {
      setError("Feil ved oppdatering av prosjekt");
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-100">
        <div className="text-xl text-gray-600">Laster prosjekt...</div>
      </div>
    );
  }
  ...
  if (!project) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-100">
        <div className="text-xl text-red-600">Prosjekt ikke funnet</div>
      </div>
    );
  }
    ...
    <div className="min-h-screen flex bg-gray-100">
      ...
      <main className="flex-1 flex flex-col min-h-screen">
        ...
        <div className="sticky top-0 z-10 bg-white border-b border-gray-200 px-8 py-4 flex items-center justify-between shadow-sm">
          <h1 className="text-2xl font-bold text-gray-900">Rediger Prosjekt</h1>
          <button
            onClick={() => router.back()}
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-gray-500">
                  La feltet være tomt for å beholde det eksisterende bildet
                </p>
              </div>
              <div>
